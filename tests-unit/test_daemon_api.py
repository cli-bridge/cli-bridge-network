...
class DaemonApiTests(unittest.TestCase):
    ...
    def test_network_connect_package_route_returns_one_shot_contract(self):
        with daemon_url() as base_url:
            ...
            self.assertEqual(payload["demo_playbook"]["step_count"], 7)
            self.assertEqual(payload["demo_playbook"]["steps"][1]["target"], "Connect")
            self.assertEqual(payload["demo_playbook"]["steps"][2]["id"], "show_sdk_bootstrap")
            self.assertIn("--output sdk-bootstrap", payload["demo_playbook"]["steps"][2]["command"])
            self.assertEqual(payload["demo_playbook"]["steps"][4]["target"], "Daemon Verify")
            self.assertIn(f"--base-url {base_url}", payload["demo_playbook"]["steps"][4]["command"])
            ...
            self.assertTrue({check["id"]: check for check in mvp["checks"]}["network_consumer_manifest"]["ready"])
            ...
            self.assertEqual(consumer_manifest["readiness"]["mvp_score"], "18/18")
    ...
    def test_network_quickstart_route_returns_first_call_payload(self):
        with daemon_url() as base_url:
            ...
            self.assertEqual(payload["acceptance"]["checks"][12]["expect"]["json.ok"], True)
            self.assertIn("--data", payload["requests"][12]["curl"])
    ...
    def test_network_readiness_route_returns_mvp_matrix(self):
        with daemon_url() as base_url:
            ...
            self.assertEqual(payload["checks"][8]["id"], "consumer_sdk_bootstrap")
            self.assertTrue(payload["checks"][8]["ready"])
            self.assertEqual(payload["checks"][9]["id"], "network_consumer_manifest")
            self.assertTrue(payload["checks"][9]["ready"])
            self.assertNotIn("contracts", payload)
    ...
    def test_network_verify_cli_runs_acceptance_against_daemon(self):
        ...
        self.assertEqual(results_by_id["consumer_manifest"]["evidence"]["json.kind"]["actual"], "NetworkConsumerManifest")
    ...
    def test_network_verify_route_runs_acceptance_against_daemon(self):
        ...
        results_by_id = {result["request_id"]: result for result in payload["results"]}
        self.assertEqual(results_by_id["consumer_manifest"]["evidence"]["json.status"]["actual"], "ready")
