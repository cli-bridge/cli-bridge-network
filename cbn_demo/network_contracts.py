...
def network_harness_agent_contract(
    *,
    ...
    bridge_routes = compact_request.get("bridge_routes") if isinstance(compact_request.get("bridge_routes"), list) else []
    bridge_message = (
        compact_request.get("bridge_message") if isinstance(compact_request.get("bridge_message"), dict) else {}
    )
...
def network_entry_profile(
    *,
    ...
    bridge_message = (
        request_plan.get("bridge_message") if isinstance(request_plan.get("bridge_message"), dict) else {}
    )
    ...
    entry_status = (
        "ready"
        if quickstart.get("status") in {"ready", "ready_without_daemon_url"} and request_plan.get("ok")
        else "needs_attention"
    )
    return {
        "apiVersion": CONNECT_API_VERSION,
